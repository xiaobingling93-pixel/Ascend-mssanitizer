...
```
./Ascend-mindstudio-sanitizer-*.run --run
```
...
> `./Ascend-mindstudio-sanitizer-*.run --install-path=./xxx --run`，即可将该运行包安装至 `xxx` 目录。

#### 2.3.3 卸载
可通过如下命令卸载：
```
./Ascend-mindstudio-sanitizer-*.run --uninstall
```
>[!NOTE]卸载路径说明   
> 默认将在 `$HOME/Ascend` 目录下卸载；若先前安装时通过 `--install-path` 指定了自定义路径，
> 则卸载时也需显式添加 `--install-path` 选项，例如：
> `./Ascend-mindstudio-sanitizer-*.run --install-path=./xxx --uninstall`。
