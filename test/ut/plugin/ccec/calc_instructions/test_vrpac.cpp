...
#include "plugin/ccec/calc_instructions/vrpac.cpp"

using namespace Sanitizer;
using std::vector;

TEST(VrpacCalculationInstructions, dump_half_and_float_type_record_expect_parse_success)
{
    ...
    __sanitizer_report_VRPAC_f16(memInfo.data(), record0.location.fileNo, record0.location.lineNo, record0.location.pc,
        reinterpret_cast<half*>(record0.dst), reinterpret_cast<half*>(record0.src), config0);
    __sanitizer_report_VRPAC_f32(memInfo.data(), record1.location.fileNo, record1.location.lineNo, record1.location.pc,
        reinterpret_cast<float*>(record1.dst), reinterpret_cast<float*>(record1.src), config1);
    __sanitizer_report_VRPAC_f16(memInfo.data(), record2.location.fileNo, record2.location.lineNo, record2.location.pc,
        reinterpret_cast<half*>(record2.dst), reinterpret_cast<half*>(record2.src), config2);
    __sanitizer_report_VRPAC_f16(memInfo.data(), record3.location.fileNo, record3.location.lineNo, record3.location.pc,
        reinterpret_cast<half*>(record3.dst), reinterpret_cast<half*>(record3.src), config3);
    __sanitizer_report_VRPAC_f32(memInfo.data(), record4.location.fileNo, record4.location.lineNo, record4.location.pc,
        reinterpret_cast<float*>(record4.dst), reinterpret_cast<float*>(record4.src), config4);
 
    auto record = reinterpret_cast<UnaryOpRecord *>(memInfo.data() + sizeof(RecordGlobalHead) + sizeof(RecordBlockHead) +
                                                    sizeof(RecordType) * 2 + sizeof(UnaryOpRecord) * 1);
    ...
    ASSERT_EQ(record->srcBlockSize, 512);
