...
#include "plugin/ccec/calc_instructions/vconcat.cpp"

using namespace Sanitizer;

TEST(VconcatCalcInstructions, dump_half_type_records_and_expect_equal)
{
    ...
    uint64_t config = 0;
    config |= ((static_cast<uint64_t>(record.repeat) & 0xFF) << 56) |
                ((static_cast<uint64_t>(record.regionRange) * 3) << 16);
    __sanitizer_report_vconcat(memInfo.data(), record.location.fileNo, record.location.lineNo, record.location.pc,
...
TEST(VconcatCalcInstructions, dump_float_type_records_and_expect_equal)
{
    std::vector<uint8_t> memInfo = CreateMemInfo();
    __gm__ RecordBlockHead *recordBlockHead = reinterpret_cast<__gm__ RecordBlockHead *>(memInfo.data() + sizeof(RecordGlobalHead));
    auto record = CreateRandomVecRegPropCoordOpRecord();
