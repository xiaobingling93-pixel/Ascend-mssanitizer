...
// #2988
SANITIZER_REPORT(vln, __ubuf__ half *dst, __ubuf__ half *src, uint64_t config)
{
 ...
 // #2991
SANITIZER_REPORT(vln, __ubuf__ half *dst, __ubuf__ half *src, uint8_t repeat,
...
// #2992
SANITIZER_REPORT(vln, __ubuf__ float *dst, __ubuf__ float *src, uint64_t config)
{
    RecordUnaryOpConfigFunc<RecordType::UNARY_OP>(EXTRA_PARAMS, dst, src, config, 8, 8, 32, 32);
}
...
#if defined(__DAV_C220__) || defined(__DAV_C220_VEC__)
// #2995
SANITIZER_REPORT(vln, __ubuf__ float *dst, __ubuf__ float *src, uint8_t repeat,
    uint16_t dstBlockStride, uint16_t srcBlockStride, uint16_t dstRepeatStride, uint16_t srcRepeatStride)
{
    RecordUnaryOpFunc<RecordType::UNARY_OP>(EXTRA_PARAMS, dst, src, repeat,
        dstBlockStride, srcBlockStride, dstRepeatStride, srcRepeatStride, 8, 8, 32, 32);
...
// #2994
SANITIZER_REPORT(vln, __ubuf__ float *dst, __ubuf__ float *src0, uint8_t repeat,
